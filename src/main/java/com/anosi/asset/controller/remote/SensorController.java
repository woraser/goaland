...
public class SensorController extends BaseRemoteController {
    ...
    public JSONObject findSensorManageData(HttpServletRequest request) throws Exception {
        // 获取boxNo
        String serialNo = request.getParameter("dust.device.serialNo");
        Device device = deviceService.findBySerialNo(serialNo);
        String result = URLConncetUtil.readAndAppendLines(URLConncetUtil.sendGet(remoteComponent.getFullPath(request.getServletPath()),
                URLConncetUtil.convertParams(request.getParameterMap(), true) + "&boxNo=" + device.getBoxSN(), remoteComponent.getHearders()));
